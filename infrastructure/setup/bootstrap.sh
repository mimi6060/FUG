...
if [ "$ACCOUNT_CREATED" = true ]; then
    ...
    # Save admin credentials to file for migrations
    ADMIN_CREDS_FILE="$SCRIPT_DIR/.admin-credentials"
    cat > "$ADMIN_CREDS_FILE" << EOF
# FUG Appwrite Admin Credentials
# Generated by bootstrap.sh on $(date)
# Keep this file secure and add to .gitignore!
APPWRITE_ADMIN_EMAIL=$ADMIN_EMAIL
APPWRITE_ADMIN_PASSWORD=$ADMIN_PASSWORD
EOF
